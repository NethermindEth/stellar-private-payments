...
use rsqlite_vfs::{
    ImportDbError, OsCallback, RegisterVfsError, SQLiteIoMethods, SQLiteVfs, SQLiteVfsFile,
    ...
    ffi::{
        ...
        SQLITE_OPEN_MAIN_JOURNAL, SQLITE_OPEN_SUPER_JOURNAL, SQLITE_OPEN_WAL, sqlite3_file,
        sqlite3_filename, sqlite3_vfs, sqlite3_vfs_register, sqlite3_vfs_unregister,
    },
    register_vfs, registered_vfs,
};
...
struct OpfsSAHPool {
    ...
    is_paused: Cell<bool>,
    /// Lock level for each open logical file; SAH ownership excludes other workers.
    open_files: RefCell<HashMap<String, i32>>,
...
impl SQLiteIoMethods for SyncAccessHandleIoMethods {
    ...
    unsafe extern "C" fn xLock(pFile: *mut sqlite3_file, level: i32) -> i32 {
        let file = SQLiteVfsFile::from_file(pFile);
        let pool = SyncAccessHandleStore::app_data(file.vfs);
        // SQLite requires weaker repeated lock requests to be no-ops.
        let mut files = pool.open_files.borrow_mut();
        let Some(current) = files.get_mut(file.name()) else {
            return rsqlite_vfs::ffi::SQLITE_IOERR;
        };
        *current = (*current).max(level);
        SQLITE_OK
    }

    unsafe extern "C" fn xUnlock(pFile: *mut sqlite3_file, level: i32) -> i32 {
        let file = SQLiteVfsFile::from_file(pFile);
        let pool = SyncAccessHandleStore::app_data(file.vfs);
        let mut files = pool.open_files.borrow_mut();
        let Some(current) = files.get_mut(file.name()) else {
    ...
    unsafe extern "C" fn xCheckReservedLock(
        ...
        let file = SQLiteVfsFile::from_file(_pFile);
        let pool = SyncAccessHandleStore::app_data(file.vfs);
        *pResOut = i32::from(
            pool.open_files
                .borrow()
    ...
    unsafe extern "C" fn xClose(pFile: *mut sqlite3_file) -> ::std::os::raw::c_int {
        ...
        if ret == SQLITE_OK {
            ...
            debug_assert!(exist.is_some(), "DB closed without open");
        }
        ret
    }
}
    ...
    C: OsCallback,
{
    ...
    unsafe extern "C" fn xOpen(
        ...
        // SAH owns the pool across workers. Also enforce one SQLite handle per
        // logical file within this worker; lock levels then have one owner.
        if !zName.is_null() {
            let name = std::ffi::CStr::from_ptr(zName).to_string_lossy();
            let pool = SyncAccessHandleStore::app_data(pVfs);
            if pool.open_files.borrow().contains_key(name.as_ref()) {
                return rsqlite_vfs::ffi::SQLITE_BUSY;
            }
        }
...
mod tests {
    ...
    use rsqlite_vfs::{VfsAppData, test_suite::test_vfs_store};
