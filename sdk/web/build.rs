...
fn repo_root_from_manifest_dir(manifest_dir: &Path) -> PathBuf {
    manifest_dir
        .ancestors()
        .nth(2)
        .expect("sdk/wasm/build.rs: expected crate at sdk/wasm")
        .to_path_buf()
}

fn read_file(path: &Path) -> Vec<u8> {
    fs::read(path)
        .unwrap_or_else(|e| panic!("sdk/wasm/build.rs: failed to read {}: {e}", path.display()))
}
...
fn copy_artifact(src: &Path, out_dir: &Path, name: &str) -> Vec<u8> {
    ...
    fs::write(out_dir.join(name), &bytes).unwrap_or_else(|e| {
        panic!(
            "sdk/wasm/build.rs: failed to write {}: {e}",
...
fn main() {
    ...
    if !circuits_out.is_dir() {
        ...
        panic!(
            "sdk/wasm/build.rs: missing circuit artifacts directory for PROFILE={profile}: {}. Run `{suggestion}` first.",
    ...
    let contents = format!(
        "\
// @generated by sdk/wasm/build.rs
