...
Source the env file, then run through the wrapper:
...
lifecycle: it builds `sdk/web/dist` when missing, serves it with CORS headers on
`E2E_STATIC_ORIGIN` (default `http://127.0.0.1:8099`), waits for readiness,
resolves `CHROMEDRIVER` from `PATH` when unset, and raises
`WASM_BINDGEN_TEST_TIMEOUT` to 600s — the wasm-bindgen default of 20s cannot
cover real proving plus testnet confirmation. A server already listening on that
origin is reused and left running.
...
test binary — but the variables must be **exported** (hence `set -a`) for the
`cargo` invocation, not merely present in the file.
