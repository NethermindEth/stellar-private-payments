...
pub struct TransactParams {
    ...
    /// External amount in stroops. See `crate::types::ExtData::ext_amount` for
    /// semantics.
...
// `crate::types::Field::try_from(ExtAmount)`, and is serialized into the
// circuit as a normal field element (Little-Endian bytes).
...
mod tests {
    ...
    fn withdraw_auto_builds_change_outputs() {
        ...
        match v {
            crate::zk::types::InputValue::Single(s) => assert!(s.starts_with("0x")),
    ...
    fn compute_test_note_commitment(
        ...
        let sender_pubkey =
            crate::zk::crypto::derive_public_key(&note_private_key.0).expect("derive public key");
        let sender_pubkey_arr: [u8; 32] = sender_pubkey.try_into().expect("public key length");
        let commitment = crate::zk::crypto::compute_commitment(
            &amount_field_le,
            &sender_pubkey_arr,
            &note_blinding_le,
        )
