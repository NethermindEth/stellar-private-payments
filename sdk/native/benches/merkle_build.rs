...
//!
//! Both call `MerklePrefixTree::new(depth, &leaves).into_built()` once per
//! transaction (`sdk/native/src/transact.rs:236` for the ASP membership tree,
//! `:313` for the pool tree), so the whole prefix is re-hashed every time.
//! Issue #167 asks how many leaves correspond to how much build time before
//! deciding whether to build once and append.
//!
...
/// does, but making them distinct avoids any chance of a hash cache flattering
/// the result.
...
fn bench_build(c: &mut Criterion) {
    // ASP membership tree and pool tree depths, read from the SDK rather than
    // assumed. Depths as actually deployed, from
    // `deployments/scripts/deploy.sh`: --asp-levels 10, --pool-levels 20. Not
    // the 8 and 10 the contract and e2e tests use.
    for (label, depth) in [("asp_membership_d10", 10u32), ("pool_d20", 20u32)] {
        ...
        for n in [0usize, 1, 16, 256, 1_024, 4_096, 16_384] {
            ...
            group.bench_with_input(BenchmarkId::from_parameter(n), &l, |b, l| {
                b.iter(|| MerklePrefixTree::new(depth, l).expect("new").into_built())
            });
        }
        group.finish();
    }
...
/// `new` on its own, to separate the empty-subtree chain from the level walk.
/// If `new` is flat and `into_built` is linear then caching the built tree is
