...
All examples run in **release mode**, because Groth16 proving is impractically
slow in a debug build. The transact examples resolve circuit artifacts from
`target/circuits-artifacts`; populate it with `make circuits-build`.
...
- Transact examples (`deposit`, `transfer`, `withdraw`) need circuit artifacts. Build them first with `./scripts/build-circuits.sh`.
