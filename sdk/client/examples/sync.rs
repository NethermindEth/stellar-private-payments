...
fn probe_bootnode(client: &stellar_private_payments_sdk::blocking::Client) -> Result<bool, String> {
    let fetcher = client
        .state_fetcher()
        .map_err(|e| format!("state fetcher: {e}"))?;
    let runtime = tokio::runtime::Runtime::new().map_err(|e| format!("tokio runtime: {e}"))?;
    runtime
        .block_on(stellar_private_payments_sdk::bootnode_required(
            fetcher.rpc(),
            client.storage(),
...
fn run_background_sync(
    background: stellar_private_payments_sdk::BackgroundSync<
        stellar_private_payments_sdk::LocalStorage,
    >,
) -> Result<(), String> {
    let runtime = tokio::runtime::Runtime::new().map_err(|e| format!("tokio runtime: {e}"))?;
    runtime
