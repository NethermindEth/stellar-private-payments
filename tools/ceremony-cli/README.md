...
- Compiled circuit (`.r1cs`). If `--circuit` is omitted the CLI auto-discovers the compiled `policy_tx_2_2_permissioned.r1cs` from `target/*/build/circuits-*/out/circuits/` (release profile preferred). Run `cargo build -p circuits --release` to compile.
...
**Current circuit (`policy_tx_2_2_permissioned`):** 37,616 constraints → requires **ptau power ≥ 16** (2^16 = 65,536).
...
ceremony-cli export-deployment \
  ...
  --basename policy_tx_2_2_permissioned \
  --force
```
...
- `deployments/testnet/circuit_keys/policy_tx_2_2_permissioned_vk.json`
- `deployments/testnet/circuit_keys/policy_tx_2_2_permissioned_vk_soroban.bin`
- `deployments/testnet/circuit_keys/policy_tx_2_2_permissioned_vk_const.rs`

Why this step exists:

- `artifacts/circuit_verification_key.json` is enough to redeploy the verifier contract.
- the app prover also needs `policy_tx_2_2_permissioned_proving_key.bin`, which is an arkworks-serialized `ProvingKey<Bn254>`, not a `snarkjs` `.zkey`
