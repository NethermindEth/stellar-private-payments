...
- Compiled circuit (`.r1cs`). If `--circuit` is omitted the CLI reads `policy_tx_2_2_AB.r1cs` from `target/circuits-artifacts/`. Run `./scripts/build-circuits.sh` to compile.
...
```bash
./scripts/build-circuits.sh           # compile circuit → produces .r1cs
...
Internally, the helper exports the `.zkey` to JSON with `snarkjs`, reconstructs the arkworks proving key, writes `proving_key.bin`, and emits the verifier artifacts in the same formats as `tools/circuit-builder`.
...
```bash
# One-time setup
./scripts/build-circuits.sh           # compile circuit (for verification)
cargo install --path tools/ceremony-cli    # install CLI to ~/.cargo/bin/
