...
use serde::Serialize;
use stellar_private_payments_sdk::{Client, types::PoolConfigEntry};

use crate::{config::CliConfig, output};

/// Drive an async SDK call to completion from the synchronous CLI.
///
/// The SDK's blocking wrapper keeps its runtime private, so chain commands own
/// a short-lived Tokio runtime for the one RPC round-trip they make.
