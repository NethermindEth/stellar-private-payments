...
/// User settings loaded from a TOML file (`~/.config/stellar-pp/config.toml` by
/// default).
...
pub fn default_config_path() -> PathBuf {
    std::env::var_os("HOME")
        .map(PathBuf::from)
        .map(|home| home.join(".config/stellar-pp/config.toml"))
        .unwrap_or_else(|| PathBuf::from("stellar-pp.toml"))
}
