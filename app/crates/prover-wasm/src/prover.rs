...
//!
//! Note: This module cannot use ark_circom directly because it depends on wasmer
//! which doesn't work in browser WASM. Instead, we:
...
use crate::r1cs::R1CS;
use crate::serialization::bytes_to_fr;
use crate::types::{Groth16Proof, FIELD_SIZE};
...
impl Prover {
    ...
    ///
    /// # Arguments
    /// * `witness_bytes` - Full witness as Little-Endian bytes (from witness calculator)
    ///
    /// # Returns
    ...
    pub fn prove(&self, witness_bytes: &[u8]) -> Result<Groth16Proof, JsValue> {
        ...
        for i in 0..num_witness_elements {
            let chunk = &witness_bytes[i * FIELD_SIZE..(i + 1) * FIELD_SIZE];
...
pub fn verify_proof(
    ...
    // Verify
    let result =
        <ark_groth16::Groth16<Bn254> as SNARK<Fr>>::verify_with_processed_vk(&pvk, &public_inputs, &proof)
            .map_err(|e| JsValue::from_str(&format!("Verification error: {}", e)))?;
