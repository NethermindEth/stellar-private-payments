...
///
/// Computes `P(left, right)[0] + left`, matching the feed-forward compression
/// used in the Circom circuits.
#[inline]
...
/// Build a Merkle root from a full power-of-two list of leaves.
///
/// # Panics
///
...
pub fn merkle_proof_internal(leaves: &[Scalar], mut index: usize) -> (Vec<Scalar>, u64, usize) {
    ...
    let levels = usize::try_from(level_nodes.len().ilog2()).expect("tree depth fits usize");
    ...
    for _level in 0..levels {
        ...
        path_indices_bits_lsb.push(u64::try_from(index & 1).expect("bit fits u64"));
