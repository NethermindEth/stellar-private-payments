...
impl Storage {
    ...
    pub fn open(path: &str) -> anyhow::Result<Self> {
        ...
        let s = Self { conn };
        s.conn.execute_batch(SCHEMA).context("applying schema")?;
        Ok(s)
    }
    ...
    pub fn get_all_public_keys(&self) -> anyhow::Result<Vec<PublicKeyEntry>> {
        ...
        stmt.query_map([], map_public_key_entry)
...
fn map_public_key_entry(row: &rusqlite::Row<'_>) -> Result<PublicKeyEntry, SqlError> {
    Ok(PublicKeyEntry {
        address: row.get(0)?,
