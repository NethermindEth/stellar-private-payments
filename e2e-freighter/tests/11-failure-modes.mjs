...
export async function run(helpers) {
  ...
  // Every money action in app/js/ui/transactions.js opens the shared
  // confirmation modal (app/js/ui/confirm.js) and returns early unless it
  // resolves true. That gate sits BEFORE the code paths this battery
  // exercises: coin selection for cases 1-2, simulateTransaction for case 4.
  // A bare button click therefore produces no toast at all — not a wrong
  // one, none — so each case has to confirm before it can observe anything.
  // The only exception is case 3, which fires on the address field's input
  // listener and never reaches a submit button.
  const confirmModal = async (title, buttonLabel) => {
  ...
  await confirmModal('Confirm withdrawal', 'Withdraw');
  ...
  await confirmModal('Confirm transfer', 'Transfer');
  ...
  await confirmModal('Confirm deposit', 'Deposit');
