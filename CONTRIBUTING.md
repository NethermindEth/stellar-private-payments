...
- **`make witness-graphs`**: Regenerates committed `*.graph.bin` witness graphs (`REGEN_GRAPHS=1` + `--features regen-graphs`). Requires Circom CLI matching `circuits/circom.lock` and a C++ toolchain.
