...
| `test/circuits/globalViewKey_{2,4}_test.circom` | Standalone encryption-only circuits for the known-answer/roundtrip tests (built with `--include-tests`). |
...
`circuit-builder keys --force`) and land in the directory given by `--keys-dir`.
